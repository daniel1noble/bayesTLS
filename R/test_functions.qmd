...
plot_survival_curves <- function(pred,
  ...
  p <- ggplot(
    ...
    scale_colour_viridis_d(option = "D", end = 0.88) +
    scale_fill_viridis_d(option = "D", end = 0.88) +
    labs(
...
plot_temperature_for_duration <- function(temp_threshold) {
  ...
  ggplot(df, aes(x = assay_temp_threshold)) +
    ...
    geom_vline(
      xintercept = median(df$assay_temp_threshold, na.rm = TRUE),
      linetype = "dashed",
      colour = "red"
    ) +
    labs(
...
make_temperature_scenarios <- function(start = as.POSIXct("2026-07-01 00:00:00", tz = "UTC"),
  ...
  mild_repairable_peak <- data.frame(
    datetime = datetime,
    temp = baseline + 0.7 * sin(2 * pi * time_h / 24)
  )
  mild_idx <- spike_start:min(n, spike_start + max(2, spike_length - 2) - 1)
  mild_repairable_peak$temp[mild_idx] <- 31.2
  mild_repairable_peak$temp[(max(mild_idx) + 1):n] <- 17 +
    0.4 * sin(2 * pi * time_h[(max(mild_idx) + 1):n] / 24)

  severe_peak <- no_heat
  severe_idx <- spike_start:min(n, spike_start + spike_length + 2)
  severe_peak$temp[severe_idx] <- spike_temp + 1.5

  list(
    no_heat = no_heat,
    mild_repairable_peak = mild_repairable_peak,
    single_spike = single_spike,
    repeated_spikes = repeated_spikes,
    severe_peak = severe_peak
  )
}
...
r2_table <- function(model_checks) {
  r2 <- model_checks$r2
  if (inherits(r2, "error")) {
    return(data.frame(Metric = "Bayesian R2", Value = NA_real_))
  }

  data.frame(
    Metric = c("Bayesian R2", "Marginal Bayesian R2"),
    Estimate = c(
      unname(r2[["R2_Bayes"]]),
      unname(r2[["R2_Bayes_marginal"]])
    ),
    `95% CI` = c(
      format_interval(
        r2[["R2_Bayes"]],
        attr(r2, "CI")$R2_Bayes$CI_low,
        attr(r2, "CI")$R2_Bayes$CI_high,
        digits = 3
      ),
      format_interval(
        r2[["R2_Bayes_marginal"]],
        attr(r2, "CI")$R2_Bayes_marginal$CI_low,
        attr(r2, "CI")$R2_Bayes_marginal$CI_high,
        digits = 3
      )
    ),
    check.names = FALSE
...
if (tdt_has_fit(fit_shrimp_4pl)) {
  ...
  tdt_ft(r2_table(model_checks), digits = 3, caption = "Bayesian R2 summaries.")
...
if (tdt_has_fit(fit_shrimp_4pl)) {
  ...
  tdt_params_table <- shrimp_tdt_params$summary %>%
    dplyr::mutate(
      ...
      `CT at 1 min (°C)` = format_interval(
        CTmax_ref_median,
        CTmax_ref_lower,
        CTmax_ref_upper,
        digits = 2
      ),
      `TDT R2` = round(r2_median, 3)
    ) %>%
    dplyr::select(`z (°C per decade)`, `CT at 1 min (°C)`, `TDT R2`)
...
The repair parameters below define scenario curves, not fitted biological constants. Damage is estimated from the static 4PL; repair is varied as a sensitivity assumption. The plots therefore answer: under these assumed repair rates, which heat pulses are predicted to accumulate injury, and which pulses are predicted to be repaired before producing substantial mortality?
...
if (tdt_has_fit(fit_shrimp_4pl)) {
  ...
  p_damage <- plot_damage_rate(damage_shrimp)
  p_repair <- plot_repair_tpc(repair_shrimp)
  p_damage_repair <- plot_damage_repair_rates(
    damage_curve = damage_shrimp,
    repair_pars = repair_pars_shrimp,
    kdot_ref_pct_min = repair_levels_pct_min[["moderate"]],
    temp_grid = seq(10, 36, by = 0.1)
  )
  p_net_damage <- plot_net_damage_rate(damage_shrimp, repair_shrimp)

  ggsave(
    filename = file.path(tdt_fig_dir, "shrimp_lethal_damage_rate.png"),
    plot = p_damage,
    width = 8,
    height = 5,
    dpi = 400
  ...
  ggsave(
    filename = file.path(tdt_fig_dir, "shrimp_lethal_repair_tpc.png"),
    plot = p_repair,
    width = 8,
    height = 5,
    dpi = 400
  ...
  ggsave(
    filename = file.path(tdt_fig_dir, "shrimp_lethal_damage_repair_overlay.png"),
...
- **Extrapolation is risky**: CTmax estimates and 1-hour tolerance temperatures lie near or outside the observed design space. Posterior intervals widen substantially in these regions; they do not make extrapolation safe.
