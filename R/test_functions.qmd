...
Unlike the prototype shrimp model, this implementation uses the built-in `brms::beta_binomial()` family and removes the observation-level random effect.
...
eta_expr <- paste0(
  ...
  formula_parts <- list(
    stats::as.formula("n_surv | trials(n_total) ~ eta"),
...
plot_temperature_for_duration <- function(temp_threshold) {
  ...
  sum_df <- temp_threshold$summary

  d_dens <- density(df$assay_temp_threshold, na.rm = TRUE)
  y_ci   <- -max(d_dens$y, na.rm = TRUE) * 0.1

  ggplot(df, aes(x = assay_temp_threshold)) +
    geom_density(fill = "steelblue", alpha = 0.35, colour = "steelblue4", linewidth = 0.5) +
    geom_segment(
      data = sum_df,
      aes(x = temp_lower, xend = temp_upper, y = y_ci, yend = y_ci),
      linewidth = 1.0,
      colour = "#2F2F2F"
    ) +
    geom_point(
      data = sum_df,
      aes(x = temp_median, y = y_ci),
      size = 2.8,
      shape = 19,
      colour = "#2F2F2F"
    ) +
...
```{r}
#| fig-cap: "**Figure 3.** Thermal death time (TDT) curves for three survival thresholds. Each line shows the posterior median time (min, log10 scale) needed to reduce survival to 75% (green), 50% (teal), or 25% (orange) at a given assay temperature; shaded ribbons are 95% credible intervals. All curves are derived by numerically inverting the posterior 4PL survival model at each temperature, and therefore fully propagate parameter uncertainty. Note that LT25 (25% survivors) requires more time than LT50 at the same temperature, as it represents greater cumulative mortality."
if (tdt_has_fit(fit_shrimp_4pl)) {
  p_ltx_all <- plot_ltx_curve(shrimp_ltx_all, y_log10 = TRUE, y_limits = c(1, 2000))

  p_lt50 <- plot_ltx_curve(shrimp_lt50, y_log10 = TRUE, y_limits = c(1, 2000))
  ...
  tdt_params_1h <- shrimp_lt1h$summary %>%
    dplyr::mutate(
      `CT for 1 h @ 50% survival (°C)` = format_interval(
        temp_median, temp_lower, temp_upper, digits = 2
      )
    ) %>%
    dplyr::select(category, `CT for 1 h @ 50% survival (°C)`)

  tdt_ft(
    cbind(tdt_params_table, tdt_params_1h),
    caption = "Table 5. Classical TDT parameters derived from the posterior LT50 curve. z is the thermal tolerance range (°C per decade of time), estimated as −1/slope from per-draw log-linear regressions of log10(LT50) on temperature. CT for 1 h @ 50% survival is the temperature at which 50% of organisms survive a 1-hour exposure, derived by numerical inversion of the posterior 4PL model."
  )
}
```
...
```{r}
#| fig-cap: "**Figure 4.** Posterior distribution of the temperature at which 50% of shrimp survive a 1-hour heat exposure. The density curve shows the full posterior distribution; the horizontal line below spans the 95% credible interval, with the filled circle marking the posterior median. This quantity is derived by numerically inverting the posterior 4PL survival model at a fixed exposure duration of 1 hour, and is conceptually analogous to a CTmax estimated at a standardised heating duration rather than at a fixed ramping rate."
if (tdt_has_fit(fit_shrimp_4pl)) {
  saveRDS(shrimp_lt1h, file.path(tdt_output_dir, "shrimp_lethal_temperature_for_1h_50_survival.RDS"))

  p_temp_1h <- plot_temperature_for_duration(shrimp_lt1h)
  ...
  temp_1h_table <- shrimp_lt1h$summary %>%
    dplyr::mutate(
  ...
  tdt_ft(temp_1h_table, caption = "Table 6. Posterior median and 95% credible interval for the temperature at which 50% of organisms survive a 1-hour exposure.")
  p_temp_1h
}
```
