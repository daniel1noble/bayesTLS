...
| `get_brmsfit()`, `get_z_draws()` / `get_z_summary()`, `get_ctmax_draws()` / `get_ctmax_summary()`, `get_tcrit_draws()` / `get_tcrit_summary()`, `get_tls_draws()` / `get_tls_summary()`, `get_surv_draws()`, `get_hi_draws()`, `has_fit()`, … | Accessors for the underlying fit, posterior draws, and posterior summaries. `get_tls_draws()` merges *z*, CTmax and (lethal) *T*~crit~ draws on `.draw` into one frame (joint pairing preserved); `get_tls_summary()` is its summary companion — one tidy row per quantity (median + 95% CrI). |
