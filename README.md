...
| `derive_z()`, `derive_tdt_curve()`, `derive_temperature_for_duration()`, `derive_tdt_landscape()` | Exported TDT primitives (the building blocks `tls()` composes, and available directly for advanced use such as the per-temperature local *z*(*T*) profile from `derive_z()`); `derive_z()` and `derive_temperature_for_duration()` are likewise group-aware (`by =`). |
