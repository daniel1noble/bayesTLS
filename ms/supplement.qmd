...
Both posterior intervals cover their analytical truths. On a bent curve, the single-summary $z$ from OLS across the assay grid estimates an average local sensitivity (truth = `r round(true_mean_local_z, 2)` °C; posterior median `r round(et_ext$z$summary$z_median, 2)` °C, 95% CrI [`r round(et_ext$z$summary$z_lower, 2)`, `r round(et_ext$z$summary$z_upper, 2)`] °C). Direct 4PL inversion at the reference duration also recovers $CT_{max_{1hr}}$ (truth `r round(true_CTmax_1hr, 2)` °C; posterior median `r round(et_ext$CTmax$summary$temp_median, 2)` °C, 95% CrI [`r round(et_ext$CTmax$summary$temp_lower, 2)`, `r round(et_ext$CTmax$summary$temp_upper, 2)`] °C), without assuming a constant asymmetry correction in $T$.
...
`extract_tdt()` returns $z$ (derived by per-draw OLS on the TDT curve) and $CT_{max_{1hr}}$ (the temperature at which the retained-function threshold is reached after 1 h, obtained by per-draw 4PL inversion).
...
#| tbl-cap: "TDT summaries for snow gum leaf PSII function, with full posterior uncertainty. $z$ comes from a per-draw log-linear regression of threshold time on temperature. $CT_{max_{1hr}}$ is the temperature at which the retained-function threshold is reached after 1 h (4PL inversion per draw)."
