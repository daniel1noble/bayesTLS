...
We can now do some manual manipulation to show how to recover the same z (absolute version) we get from each of the indiviudal models.
...
# in the time to the threshold).
z_F <- -1 / slope_F
z_M <- -1 / slope_M

# Posterior median and 95% CI per sex (compare with the per-sex models above).
...
Everything above - $z$,
$CT_{max_{4hr}}$ and $T_{crit}$ — can be calculated using a single `tls()` call 
on any custom `brms` 4PL: it evaluates each sub-parameter at the requested
moderator grid with `posterior_linpred()` (just like you would predict with any brms object). Moderators on *any* 4PL parameter
(and random effects) are handled automatically.
...
# long summary (one row per sex x quantity).
tls_sex_summary <- tls(joint_sex_fit, by = "sex", lethal = TRUE, t_ref = 60 * 4,
...
The joint-model summary (@tbl-dros-sex-joint) matches the by-hand values above ($z$ 3.04 / 3.18, $CT_{max_{4hr}}$ 35.1 / 35.2 °C; $T_{crit}$ ~29 °C).


# **Manuscript Figure 4: cross-case-study summary** {#sec-ms-fig4}

This figure compiles the headline TDT quantities ($z$ and $CT_{max_{1hr}}$) from the three case studies into a single multi-panel display, intended for the main manuscript. Each panel overlays the joint Bayesian 4PL posterior (green ridge + green median/95% CrI) with the classical two-stage point estimate (orange median/95% CI) at the matched Stage-1 likelihood — beta-binomial for the count-based case studies (shrimp, zebrafish), beta for the proportional case study (snow gum leaf PSII). The zebrafish row splits into three ridges, one per life stage.

```{r ms-fig4-case-study-densities}
#| label: fig-ms-fig4-case-study-densities
#| fig-cap: "Manuscript Figure 4. Posterior densities of $z$ (left column) and $CT_{max_{1hr}}$ (right column) from the joint Bayesian 4PL fits (green ridges, #006d2c) for the three case studies (rows). The 4PL posterior median and 95% credible interval are overlaid as a green pointrange sitting at the density baseline; the two-step classical estimate (median + 95% CI, orange #a63603) is shown directly below as an orange pointrange — beta-binomial Stage 1 for the brown shrimp and zebrafish counts, beta Stage 1 for the snow gum leaf PSII proportions. Two-step $z$ intervals are obtained by inverting the Stage-2 slope's 95% CI; intervals are NA (no orange bar) when the slope CI crosses zero, signalling an unidentified Stage-2 slope. Dashed vertical lines mark the 4PL posterior median for each entity. The zebrafish row splits into three ridges, one per life stage. Each case study has independent x-axes in both columns. $z$ is in °C (it is the temperature change required for a 10-fold change in time to the threshold)."
#| fig-width: 9
#| fig-height: 7
#| cache: false
