...
We see that our *z* values (their Table 1: 3.28 for males, 3.03 for females) as well as $CT_{max_{4hr}}$ (35.2 °C for both sexes) pretty much match @orsted_suzukii_2024 quite well for mortality. 
...
Everything above — the asymmetry correction, the per-sex slopes, and $z$,
$CT_{max_{4hr}}$ and $T_{crit}$ — is bundled into a single `tls()` call that works
on any custom `brms` 4PL: it evaluates each sub-parameter at the requested
moderator grid with `posterior_linpred()`, so moderators on *any* 4PL parameter
(and random effects) are handled automatically, with no coefficient bookkeeping.
...
#| code-fold: false
set.seed(123)
tls(joint_sex_fit, by = "sex", lethal = TRUE, t_ref = 60 * 4,
    temp_mean = mean(std_sex$temp - std_sex$temp_c))$summary
```

These match the by-hand values above ($z$ 3.04 / 3.18, $CT_{max_{4hr}}$ 35.1 / 35.2 °C; $T_{crit}$ ~29 °C).
