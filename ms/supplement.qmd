...
shrimp_scens <- make_temperature_scenarios(
  ...
  spike_times_single = 24,
  spike_times_multi  = c(24, 48, 72),
  # Diurnal: 7 days, night below T_crit (no injury), day peaks alternating
  # cool (≤ T_crit, no injury) and warm (above T_crit, ~25% HI per day).
  diurnal_n_days     = 7,
  diurnal_night_temp = 19,
  diurnal_day_peaks  = c(24, 26.5, 24.5, 27, 23, 26.5, 25.5),
  diurnal_noise_sd   = 0.25,
  diurnal_seed       = 1L
)
...
#| fig-cap: "The four reference temperature traces used for the shrimp heat-injury prediction. Baseline 17 °C is the natural holding temperature; the 1-hour spikes are set to the posterior median $CT_{max_{1hr}}$ (LT50 temperature) so each spike delivers a full LT50 dose by construction. The diurnal scenario alternates cool days (peaks at or below $T_{crit}$, no injury) with warm days (peaks above $T_{crit}$, ~25% LT50-dose per day). The dashed line marks the model-derived damage threshold $T_c = T_{crit}$."
#| fig-width: 7
#| fig-height: 6
...
The diurnal scenario (@fig-shrimp-hi-diurnal-norep, @fig-shrimp-hi-diurnal-rep) is more realistic: seven 24-hour day–night cycles with day-to-day variation in the peak temperature, plus small hourly fluctuations. Some days' peaks stay below $T_{crit}$ (no injury accrues); on warm days the peak rises above $T_{crit}$ and a sub-LT50 dose accumulates. The no-repair trajectory shows monotonic accumulation on the warm-day peaks, separated by flat plateaus on the cool days. With repair on, the cool-day plateaus turn into recovery phases — cumulative HI falls back as the organism repairs faster than it accrues new injury at sub-$T_{crit}$ temperatures.

```{r shrimp-hi-diurnal-noreapair}
#| label: fig-shrimp-hi-diurnal-norep
#| fig-cap: "Cumulative heat injury (top) and predicted survival (bottom) for brown shrimp under the **diurnal** trace, **no repair**. Peaks on the warm days push the temperature above $T_{crit}$ and drive injury accumulation; cool days flat-line. Posterior median + 95% credible band."
#| fig-width: 7
#| fig-height: 5
...
#| fig-cap: "Same diurnal trace, this time **with Sharpe-Schoolfield repair** active (`r_ref = 0.05` doses per hour at 17 °C). Cool days now turn into recovery phases, repair offsets some of the previous warm-day damage, and the trajectory is dominated by the net balance between daily injury accrual and overnight repair."
#| fig-width: 7
#| fig-height: 5
