...
```{r shrimp-two-step-lt50-table}
#| label: tbl-shrimp-two-step-lt50
#| tbl-cap: "Stage-1 shrimp two-stage estimates: per-temperature $\\log_{10}\\text{LT50}$ in the model time unit (hours). The beta-binomial column also reports the fitted precision parameter $\\phi$; larger $\\phi$ approaches the binomial likelihood."

shrimp_stage1_twostep |>
  dplyr::mutate(
    `Temperature (C)` = temp,
    `Stage-1 likelihood` = method,
    `log10 LT50 (hours)` = log10_lt50,
    `Beta-binomial phi` = phi,
    `Used in Stage 2` = stage1_ok
  ) |>
  dplyr::select(`Temperature (C)`, `Stage-1 likelihood`,
                `log10 LT50 (hours)`, `Beta-binomial phi`,
                `Used in Stage 2`) |>
  tinytable::tt(digits = 3, escape = TRUE)
```
...
#| tbl-cap: "Classical two-stage TDT quantities for brown shrimp. These are point estimates from the Stage-2 regression; $T_{crit}$ uses the geometric midpoint of the same $r^* \\in [0.1, 1]$ % HI per hour range used for the joint 4PL extraction."

shrimp_twostep_tdt |>
  dplyr::transmute(
    `Stage-1 likelihood` = method,
    `z (C)`              = z,
    `CTmax_1hr (C)`      = CTmax_1hr,
    `T_crit (C)`         = T_crit,
    `Stage-2 R2`         = r_squared,
    `Stage-1 estimates used` = n_stage1,
    `Stage-1 estimates excluded` = n_excluded
  ) |>
  tinytable::tt(digits = 3, escape = TRUE)
```
...
#| fig-cap: "Classical two-stage TDT lines for brown shrimp. Points are Stage-1 $\\log_{10}\\text{LT50}$ estimates; lines are the Stage-2 regressions. The beta-binomial Stage-1 fit allows overdispersion within each assay temperature, but both approaches reduce to a single point estimate per temperature for Stage 2."
#| fig-width: 7
#| fig-height: 4.5
...
#| tbl-cap: "Brown shrimp TDT quantities from the classical two-stage pipeline and the joint Bayesian 4PL. Two-stage rows are point estimates; the 4PL row reports posterior medians with 95 % credible intervals."
...
shrimp_compare_approaches <- dplyr::bind_rows(
  shrimp_twostep_tdt |>
    dplyr::transmute(
      Approach = method,
      `z (C)` = sprintf("%.2f", z),
      `CTmax_1hr (C)` = sprintf("%.2f", CTmax_1hr),
      `T_crit (C)` = sprintf("%.2f", T_crit)
    ),
  tibble::tibble(
...
#| fig-cap: "Brown shrimp TDT-line comparison. The joint 4PL posterior median and 95 % credible band are repeated from @fig-shrimp-tdt-line; the two classical two-stage lines are overlaid as point-estimate regressions."
#| fig-width: 9
#| fig-height: 4
...
```{r zebrafish-two-step-lt50-table}
#| label: tbl-zebrafish-two-step-lt50
#| tbl-cap: "Stage-1 zebrafish two-stage estimates: per-temperature $\\log_{10}\\text{LT50}$ in the model time unit (hours), by life stage and Stage-1 likelihood."

zf_stage1_twostep |>
  dplyr::group_by(life_stage, method) |>
  dplyr::summarise(
    `Stage-1 curves fit` = dplyr::n(),
    `Used in Stage 2` = sum(stage1_ok, na.rm = TRUE),
    `Excluded from Stage 2` = sum(!stage1_ok, na.rm = TRUE),
    `Median beta-binomial phi` = stats::median(phi, na.rm = TRUE),
    .groups = "drop"
  ) |>
...
#| tbl-cap: "Classical two-stage TDT quantities for zebrafish, by life stage. These are point estimates from the Stage-2 regression; $T_{crit}$ uses the geometric midpoint of the same $r^* \\in [0.1, 1]$ % HI per hour range used by the 4PL extraction."
...
#| fig-cap: "Classical two-stage TDT lines for zebrafish, by life stage. Points are Stage-1 $\\log_{10}\\text{LT50}$ estimates; lines are the Stage-2 regressions. Solid and dashed lines distinguish binomial and beta-binomial Stage-1 likelihoods."
#| fig-width: 9
#| fig-height: 5
...
#| cache: false
#| tbl-cap: "Zebrafish TDT quantities from the classical two-stage pipeline and the joint Bayesian 4PL. Two-stage rows are point estimates; the 4PL rows report posterior medians with 95 % credible intervals."
...
zf_twostep_compare_rows <- zf_twostep_tdt |>
  dplyr::transmute(
    life_stage,
    Approach = method,
    `z (C)` = sprintf("%.2f", z),
...
#| cache: false
#| fig-cap: "Zebrafish TDT-line comparison between the joint 4PL and the classical two-stage fits. The joint 4PL posterior median and 95 % credible band are repeated from @fig-zebrafish-overlay-tdt-line; two-stage lines are point-estimate regressions from binomial and beta-binomial Stage-1 fits."
#| fig-width: 10
#| fig-height: 4.5
...
zf_twostep_joint_overlay <- dplyr::bind_rows(
  ...
  zf_twostep_line_min |>
    dplyr::transmute(
      ...
      life_stage,
      time_med_min = duration_median,
      time_lo_min = NA_real_,
      time_hi_min = NA_real_,
      approach = method
    )
) |>
  dplyr::mutate(
...
build_zf_twostep_overlay <- function(y_log = FALSE) {
  p <- ggplot2::ggplot(
    ...
    ggplot2::geom_ribbon(
      data = dplyr::filter(zf_twostep_joint_overlay,
                           approach == "Joint Bayesian 4PL"),
