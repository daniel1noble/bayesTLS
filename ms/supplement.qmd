...
## Loading the function library {#sec-load-library}

The supplement assumes the project repository is the current working directory, so [`here::here()`](https://here.r-lib.org) can resolve paths to `R/`. The chunk below loads required packages and sources every file in the function library.

```{r load-tdt-library}
#| label: load-tdt-library
#| echo: true
#| cache: false
...
tdt_function_tour <- tibble::tribble(
  ...
  "Plotting",         "plot_temperature_density()",         "Posterior density for CTmax / T_crit with a 95% CrI bar.",
...
tinytable::tt(tdt_function_tour)
```

## Testing {#sec-library-tested}
...
val_table$`Truth in CrI?` <- with(val_table,
                                  `Lower (%)` <= `Planted HI (%)` &
                                  `Planted HI (%)` <= `Upper (%)`)
tinytable::tt(val_table, digits = 2)
```
...
tinytable::tt(shrimp_summary, digits = 2)
```
...
tinytable::tt(diagnose_tdt_fit(wf_shrimp), digits = 3)
```
...
tinytable::tt(tdt_parameter_table(wf_shrimp), digits = 3)
```
