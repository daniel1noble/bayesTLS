...
tdt_function_tour <- tibble::tribble(
  ...
  "Plotting",         "plot_survival_curves()",             "Posterior curves with observed proportion overlay; `clip_to_observed = TRUE` limits each temperature/group curve to its observed duration range.",
...
# clip_to_observed keeps each temperature's curve within its tested duration
# range (aphids were exposed for far shorter times at hotter temperatures).
...
The companion package [`freqTLS`](https://github.com/itchyshin/freqTLS) fits the same models by maximum likelihood instead of MCMC, and mirrors the `bayesTLS` interface on purpose: `standardize_data()`, `fit_4pl()` and `tls()` take the same arguments and return the same tidy-long summaries — so the *same* code fits the model, only the package changes. We refit the three-species aphid model from Case Study 2 (@sec-aphids) and compare. Because `freqTLS` shares `bayesTLS`'s function names, load it **after** your `bayesTLS` analyses (or call it as `freqTLS::`); here it is the final step, so a plain `library()` is safe.
