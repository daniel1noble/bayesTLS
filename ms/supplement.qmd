...
When shape parameters vary with $T$ the closed-form expressions of @sec-joint-derive no longer apply directly — but the same `extract_tdt()` call still works, because internally it inverts the 4PL surface numerically per posterior draw. The implementation: predict survival on a dense $(T, t)$ grid, find the duration where each draw crosses the target survival (via `approx()`), then either fit a per-draw log-linear regression on `(T, log10 LT50)` for $z$, or invert again on temperature for $CT_{max_{1hr}}$. `extract_tdt()` returns both directly.

```{r extension-helper}
#| label: extension-helper
#| echo: true

# Same call as @sec-joint-derive; no special handling needed even if the
# fit had T-varying shape parameters. The internal grid evaluation is what
# makes this robust to bent log10(LT50)(T) curves.
et_bin$z$summary
et_bin$CTmax$summary
```

The posterior medians match the closed-form derivations from @sec-joint-derive — exactly as expected for this fit (linear `mid(T)`, constant shape parameters). When the same call is applied to a fit that lets shape parameters vary with $T$ (see @sec-joint-extend-example below), the same machinery produces a noticeably different $z$ posterior because the underlying $\log_{10}\text{LT50}(T)$ curve is bent rather than straight.
...
# Predict-and-interpolate recipe applied directly to the extended brmsfit
# (which was built with its own column names log10_t / T_c — see bform_ext).
# This runs inline rather than through extract_tdt() because the column
# schema differs from what standardize_data() produces; it is the same
# numerical inversion idea applied to a hand-rolled brmsfit.
T_eval    <- temps
