...
and $CT_{max}$. The main manuscript figures
([Fig. @fig-sims] and [Fig. @fig-sims2]) show bias and coverage for the
two shape-perturbing scenarios (drifting upper asymptote `u`, varying
slope `k`) and the bias results from the two partial-mortality sweeps.
...
This complements [Fig. @fig-sims2]a (which showed bias only) with the
matching coverage panels.
...
#| fig-width: 8
#| fig-height: 4.5
#| fig-cap: "Empirical 95% CI coverage across the $\\beta_u$ sweep (Scen 6). Left: $z$. Right: $CT_{max,1h}$. Dashed line at nominal 0.95. Same DGP and method set as [Fig. @fig-sims2]a, with coverage now shown for all six methods (Normal- and t-quantile two-stage CIs differ on coverage even though their point estimates are identical)."
...
Complements [Fig. @fig-sims2]b. With a low $u_0$, the two-stage
Stage-1 GLMs miscalibrate per-temperature, the Stage-2 OLS inherits
that bias, and CI coverage on $CT_{max,1h}$ collapses well below
nominal even after the t-quantile small-sample correction.
...
#| fig-width: 8
#| fig-height: 4.5
#| fig-cap: "Empirical 95% CI coverage across the $u_0$ sweep (Scen 7). Same DGP and method set as [Fig. @fig-sims2]b. Two-stage CTmax coverage collapses as $u_0$ falls; the t-correction widens CIs without recovering nominal calibration. Joint 4PL coverage holds at 92–96% throughout."
