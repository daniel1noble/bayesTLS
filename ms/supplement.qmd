...
#  - z: closed-form CI by inverting the slope's CI from `confint()`.
#  - CTmax_1hr and T_crit (both ratios of normals): parametric simulation
#    from MVN(coef, vcov), 1000 draws.
...
propagate_twostep_case <- function(stage2_fit, temp_grid,
  ...
  z_lower <- as.numeric(-1 / beta_ci[1, 2])
  z_upper <- as.numeric(-1 / beta_ci[1, 1])
...
#| tbl-cap: "Brown shrimp TDT quantities from the classical two-stage pipeline and the joint Bayesian 4PL. Two-stage rows report point estimates with 95% intervals derived from the Stage-2 linear model only — $z$ from inverting the slope CI, $CT_{max_{1hr}}$ and $T_{crit}$ from 1000 parametric simulations of the Stage-2 MVN coefficient distribution. Stage-1 LT50 uncertainty is not propagated, mirroring how classical two-step TDT estimates are reported in the literature. The 4PL row reports posterior medians with 95% credible intervals."
...
#| cache: false
#| tbl-cap: "Zebrafish TDT quantities from the classical two-stage pipeline and the joint Bayesian 4PL. Two-stage rows report point estimates with 95% intervals derived from the Stage-2 linear model only — $z$ from inverting the slope CI, $CT_{max_{1hr}}$ and $T_{crit}$ from 1000 parametric simulations of the Stage-2 MVN coefficient distribution. Stage-1 LT50 uncertainty is not propagated, mirroring how classical two-step TDT estimates are reported in the literature. The 4PL rows report posterior medians with 95% credible intervals."
...
#| fig-cap: "Snow gum leaf PSII TDT-line comparison at a common absolute threshold ($F_v/F_m = 0.5$). The joint 4PL posterior median and 95% credible band are the time to $F_v/F_m = 0.5$ (`target_surv = \"absolute\"`) — the only threshold the forced-0/1 two-stage can target; the two classical two-stage lines are overlaid with their pointwise 95% confidence bands from `predict.lm()` on the Stage-2 regression (Stage-1 LT50s treated as known points). Outside the assayed 28–48 °C window both methods extrapolate."
#| fig-width: 9
#| fig-height: 4
