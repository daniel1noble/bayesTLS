...
#| tbl-cap: "Snow gum leaf PSII TDT quantities from the classical two-stage pipeline and the joint Bayesian 4PL. Two-stage rows report point estimates with 95% intervals derived from the Stage-2 linear model only — $z$ from inverting the slope CI, $CT_{max_{1hr}}$ from 1000 parametric simulations of the Stage-2 MVN coefficient distribution. Stage-1 LT50 uncertainty is not propagated, mirroring how classical two-step TDT estimates are reported in the literature. The 4PL row reports posterior medians with 95% credible intervals."
...
leaf_compare_approaches <- dplyr::bind_rows(
  ...
  tibble::tibble(
    Approach = "Joint Bayesian 4PL",
    `z (C)` = format_interval(et_leaf$z$summary$z_median,
                              et_leaf$z$summary$z_lower,
                              et_leaf$z$summary$z_upper,
                              digits = 2),
    `CTmax_1hr (C)` = format_interval(et_leaf$CTmax$summary$temp_median,
                                      et_leaf$CTmax$summary$temp_lower,
                                      et_leaf$CTmax$summary$temp_upper,
                                      digits = 2)
  )
...
#| fig-cap: "Snow gum leaf PSII TDT-line comparison. The joint 4PL posterior median and 95% credible band are repeated from @fig-leaf-tdt-line; the two classical two-stage lines are overlaid with their pointwise 95% confidence bands from `predict.lm()` on the Stage-2 regression (Stage-1 LT50s treated as known points)."
#| fig-width: 9
#| fig-height: 4
...
leaf_4pl_line <- ltx_leaf$summary |>
  dplyr::transmute(temp,
                   duration_median,
                   duration_lower,
                   duration_upper,
...
p_leaf_cmp_lin <- ggplot2::ggplot() +
  ...
  ggplot2::labs(x = "Assay temperature (C)",
                y = "Time to 50% PSII impairment (min)",
                colour = "Approach", fill = "Approach") +
  ggplot2::theme_classic()

p_leaf_cmp_log <- p_leaf_cmp_lin +
  ggplot2::scale_y_log10() +
  ggplot2::labs(y = expression(log[10]~"time to 50% PSII impairment (min)")) +
  ggplot2::theme(legend.position = "none")
