...
# form derivations from the main text to get z and CTmax at 1 hour. The raw
# posterior columns live on the unconstrained reparameterised scale; they are
# transformed back to the natural 4PL scale inside the function.
derive_z_ctmax <- function(fit) {
  posterior::as_draws_df(fit) |>                              # <1>
    dplyr::mutate(
      ell       = 0.001 + plogis(b_lowraw_Intercept) * 0.498,
      u         = 0.501 + plogis(b_upraw_Intercept)  * 0.498,
      k         = exp(b_logk_Intercept),
      z         = -1 / b_mid_temp_c,                          # <2>
      alpha     = b_mid_Intercept +                           # <3>
        (1 / k) * log((u - 0.5) / (0.5 - ell)) -
        b_mid_temp_c * true_params$T_bar,
      CTmax_1hr = (log10(60) - alpha) / b_mid_temp_c          # <4>
    ) |>
