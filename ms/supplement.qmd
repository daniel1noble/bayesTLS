...
tdt_function_core <- tibble::tribble(
  ...
  "Extract",      "tls()",                       "Derive z, CTmax and (for lethal endpoints `lethal = TRUE`) T_crit per moderator group from ANY fitted 4PL, including hand-coded `brms` models (`by =`). Relative threshold is default by any absolute threshold can be used by modifying `target_surv` (`relative`, `absolute`, or any numeric threshold)" ,
...
# get_tls_est(., "draws") -> tidy-long draws; pivot to wide so each row is one
# posterior draw with z / CTmax / T_crit as columns, jointly paired on `.draw`.
tl_bin <- tls(wf_bin, target_surv = "relative", t_ref = 60, lethal = TRUE, ndraws = 1000)
...
# The matching summary is get_tls_est(., "summary") -- the same tidy table as
# tls()$summary above (same quantities, same threshold).
...
The same `fit_4pl()` fit, read through `tls()` and the `derive_*()` primitives, recovers the bent $\log_{10}\text{LT50}(T)$ curve, the assay-averaged $z$, and per-draw $CT_{max_{1hr}}$. Under the constant-shape truth in @sec-joint-derive, near-zero slopes on `up`, $\ell$, and $k$ reduce this calculation to the closed-form case.
